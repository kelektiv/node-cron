(function(root, factory) {
	...
	CronTime.prototype = {
		...
		/**
		 * accept json string instead of cron string
		 */
		parseJson: function() {
			const params = JSON.parse(this.source);
			timeUnits.forEach((unit, index) => {
				this._parseField(
					parseAlias(params[unit]) || CronTime.parseDefaults[index],
					unit,
			...
			// check if alias exists. If it doesn't, continue using the value as is
