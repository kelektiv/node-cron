(function(root, factory) {
	if (typeof define === 'function' && define.amd) {
		...
		moment.prototype.safeAdd=function(value,type){
			var currentTz= this.tz();
			if(currentTz){
				this.tz('UTC');
				this.add(value,type);
				this.tz(currentTz);
	...
	CronTime.prototype = {
		...
		_getNextDateFrom: function(start, zone) {
			...
			/*
			 ...
			 note: *5 is added to support  cron string that only occur in
			 leap years which occur every 4 year (12*4) such as: * * 29 1 *
			 and any other case that we might have missed.
			*/
			var MAX_ITERATIONS = (11 + 30 + 23 + 59 + 59) * 5;
			var i = 0;
			// determine next date
			const dateTz=date.tz() || "UTC";
			while (true) {
				i++;
				var prevMonth = date.month();
				var prevDay = date.days();
				var prevMinute = date.minutes();
				var prevHour = date.hours();
				var prevSecond = date.seconds();
				if (i > MAX_ITERATIONS) {
					throw new Error(
...
Time Zone: ${zone || this.zone || '""'} - Cron String: ${this} - UTC offset: ${date.format('Z')} - current Date: ${moment().toString()}`
					);
				}
				if (
				...
				if (
					...
					date.safeAdd(1,'m');
					if(prevMinute === date.minutes()){
						;
						date.safeAdd(1,'m')
						date.tz(zone || this.zone)
					}
					date.seconds(0);
					continue;
				}
