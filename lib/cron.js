var exports,
    ...
    loadedMomentModule = false,
    moment = require("moment-timezone"),
    exports = module.exports,
    loadedMomentModule = true;
...
CronTime.prototype = {
     ...
     */
    sendAt: function() {
        var date = !!this.source.tz ? this.source : moment();
        if (this.zone)
     ...
     */
    getTimeout: function() {
        return Math.max(-1, this.sendAt().valueOf() - moment().valueOf());
    },
    ...
    _getNextDateFrom: function(start) {
        ...
        while (1) {
            ...
            if (!(date.hours() in this.hour)) {
                origDate = moment(date);
                date.hours(date.hours() == 23 && diff > 24 * 60 * 60 * 1000 ? 0 : date.hours() + 1);
                date.minutes(0);
                date.seconds(0);
                if (date <= origDate) {
                    date = this._findDST(origDate);
    ...
    _findDST: function(date) {
        var newDate = moment(date),
            addSeconds = 1;
        while (newDate <= date) {
            newDate.seconds(date.seconds() + addSeconds++);
        }
