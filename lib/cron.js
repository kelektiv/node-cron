...
function CronJob(cronTime, onTick, onComplete, startNow, timeZone, context) {
	var executeNow; // only works when combined with startNow
	if (typeof cronTime != "string" && arguments.length == 1) {
		...
		executeNow = cronTime.executeNow;
	}
	...
	if (startNow) start.call(this, executeNow);
...
var start = function(executeNow) {
	...
	if (executeNow) {
		this.running = true;
		this._timeout = setTimeout(callbackWrapper, 0);
	}
	else if (timeout >= 0) {
		this.running = true;
