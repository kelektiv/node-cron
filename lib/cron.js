(function(root, factory) {
	...
	CronTime.prototype = {
		_verifyParse: function() {
			...
			for (var i = 0; i < months.length; i++) {
				...
				for (var j = 0; j < dsom.length; j++) {
					...
					if (dom <= con) {
						...
						deleteDom.push(dom);
						this.dayOfMonth[String(Number(dom)) % con] = true;
					}
				}
			...
			maxRemainDom = (Math.max.apply(null, remainDom));
			uniqueDeleteDom = deleteDom.filter(function(dom, i) {
				return deleteDom.indexOf(dom) === i;
			});

			uniqueDeleteDom.forEach(function(dDom) {
				if (maxRemainDom < dDom) {
					delete that.dayOfMonth[dDom];
				}
			});
		},
