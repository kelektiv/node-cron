(function(root, factory) {
	...
	CronTime.prototype = {
		...
		parseJson: function() {
			const { hour, minute, second, dayOfWeek, dayOfMonth, month } = JSON.parse(
				this.source
			);

			const parsedDayOfWeek =
				dayOfWeek && dayOfWeek.length === 3
					? this.aliases[dayOfWeek].toString()
					: dayOfWeek;

			const parsedMonth =
				month && month.length === 3 ? this.aliases[month].toString() : month;

			this._parseField(second || '*', 'second', CronTime.constraints[0]);
			this._parseField(minute || '*', 'minute', CronTime.constraints[1]);
			this._parseField(hour || '*', 'hour', CronTime.constraints[2]);
			this._parseField(
				dayOfMonth || '*',
				'dayOfMonth',
				CronTime.constraints[3]
			);
			this._parseField(parsedMonth || '*', 'month', CronTime.constraints[4]);
			this._parseField(
				parsedDayOfWeek || '*',
				'dayOfWeek',
				this.constraints[5]
