(function(root, factory) {
	...
	function CronJob(
		...
		if (typeof cronTime !== 'string' && argCount === 1) {
			...
			maxJobCount = cronTime.maxJobCount;
		}
		...
		this.jobFinishedCount = 0; // Initilise the job executed count
	...
	var start = function() {
		...
		function callbackWrapper() {
			if (self.maxJobCount === self.jobFinishedCount) {
				self.stop();
				return;
			}
			self.jobFinishedCount++;
