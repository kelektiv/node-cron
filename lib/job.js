function CronJob(CronTime, spawn) {
	...
	const addCallback = function (callback) {
		if (typeof callback === 'function') {
		  ...
		  throw new Error('Callback must be a function.'); // Throw an error if the callback is not a function
		}
	  };
	...
	// Replacing the fireOnTick declaration with the following code:

CJ.prototype.fireOnTick = function () {
	const self = this; // Store the reference to 'this' in a variable
	this._callbacks.forEach(function (callback) {
	  callback.call(self.context, self.onComplete); // Use the stored 'self' instead of 'this'
	});
  };
...
//replacing above function with below one

function _setTimeout(timeout) {
	const startTime = Date.now();
	const self = this;
	self._timeout = setTimeout(callbackWrapper, timeout);
	if (self.unrefTimeout && self._timeout.unref) {
	  self._timeout.unref();
	}
  }
	...
	// replacing CJ.prototype.stop function with below function.
